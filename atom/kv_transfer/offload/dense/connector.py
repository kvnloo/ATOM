    ...
    def _do_load_req(self, req: LMCacheReqMeta) -> None:
        ...
        if self._profile_enabled():
            logger.info(
                ...
                float(transfer_stats.get("pack_ms", -1.0)),
                float(transfer_stats.get("copy_ms", -1.0)),
                float(transfer_stats.get("sync_ms", -1.0)),
                float(transfer_stats.get("transfer_ms", -1.0)),
                float(transfer_stats.get("effective_gbps", -1.0)),
                retrieve_ms,
                total_ms,
            )
